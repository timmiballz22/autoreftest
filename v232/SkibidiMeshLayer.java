...
public final class SkibidiMeshLayer<M extends EntityModel<? super SkibidiRenderState>>
    extends RenderLayer<SkibidiRenderState, M> {

    private static final Identifier HEAD_TEXTURE =
        SkibidiToiletMod.id("textures/entity/skibidi_head.png");
    private static final Identifier INTERIOR_TEXTURE =
        SkibidiToiletMod.id("textures/entity/skibidi_interior.png");
    private static final Identifier TOILET_TEXTURE =
    ...
    private static final RenderType HEAD_RENDER = RenderTypes.entityCutoutNoCull(HEAD_TEXTURE);
    private static final RenderType INTERIOR_RENDER = RenderTypes.entityCutoutNoCull(INTERIOR_TEXTURE);
    private static final RenderType TOILET_RENDER = RenderTypes.entityCutoutNoCull(TOILET_TEXTURE);
    ...
    public SkibidiMeshLayer(
        RenderLayerParent<SkibidiRenderState, M> parent,
        float modelScale,
    ...
    public void submit(
        ...
        submitGroup(submitNodeCollector, poseStack, TOILET_RENDER, SkibidiMesh.TOILET,
            light, overlay, 0.0F, 0.0F, false);
        submitGroup(submitNodeCollector, poseStack, HEAD_RENDER, SkibidiMesh.HEAD_BASE,
            light, overlay, yaw, pitch, true);
        submitGroup(submitNodeCollector, poseStack, INTERIOR_RENDER, SkibidiMesh.INTERIOR,
            light, overlay, yaw, pitch, true);

        if (variant.hasGlowingEyes()) {
            submitGroup(submitNodeCollector, poseStack, YELLOW_RENDER, SkibidiMesh.EYES,
                FULL_BRIGHT, overlay, yaw, pitch, true);
            submitGroup(submitNodeCollector, poseStack, YELLOW_RENDER, SkibidiMesh.PUPILS,
                FULL_BRIGHT, overlay, yaw, pitch, true);
        } else {
            submitGroup(submitNodeCollector, poseStack, HEAD_RENDER, SkibidiMesh.EYES,
                light, overlay, yaw, pitch, true);
            submitGroup(submitNodeCollector, poseStack, HEAD_RENDER, SkibidiMesh.PUPILS,
                light, overlay, yaw, pitch, true);
        }
